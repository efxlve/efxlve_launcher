...
- Giriş ekranına yeni kullanıcılar için kristal netliğinde **3 Adımlı Hızlı Başlangıç Rehberi** (`.auth-guide-box`) eklendi:
  ...
  - Adım 2: Kodu Kopyalayın (Açılan siyah sayfadaki tüm metni `Ctrl+A` ile veya sadece `authorizationCode` kodunu kopyalayın)
  - Adım 3: Buraya Yapıştırın (Launcher'daki kutucukta 'Yapıştır' butonuna basıp giriş okuna tıklayın)
- `extractAuthCode` ayrıştırıcısı akıllı hale getirildi (`src/features/auth/auth-actions.ts`): Kullanıcı tarayıcıdaki tüm JSON çıktısını seçip kopyalasa (`Ctrl+A`), adres çubuğundaki `redirectUrl` bağlantısını yapıştırsa veya tırnak işaretleriyle kopyalasa dahi yetkilendirme kodunu hatasız ayıklar.
- Giriş kartı eylem mimarisi optimize edildi: En üstte tek tıkla şifresiz çalışan büyük "Açık Epic Games Launcher Oturumunu Aktar" butonu, ortada ince "VEYA" ayracı, altında web oturumu butonu, kod yapıştırma kapsülü ve 3 adımlı rehber konumlandırıldı.
