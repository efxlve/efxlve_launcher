...
- Epic Games Launcher açık olmasına rağmen aktarımın hata vermesinin (`ValueError: EGS AppData path does not exist`) teknik nedeni teşhis edildi: Modern Epic Games Launcher sürümlerinin (2024–2026) eski `%LOCALAPPDATA%\EpicGamesLauncher\Saved\Config\Windows\GameUserSettings.ini` düz metin dosyasını kullanmayıp Chromium CEF `webcache_4430` ve DPAPI şifreli depolamaya geçmesi sebebiyle upstream Legendary CLI'ın bu dosya yolunu bulamamasıdır. Bu sebeple "Epic Games ile Giriş Yap" (Web Auth) akışının en kararlı ve resmi yöntem olduğu netleştirildi.
