...
- **Hesap geçişi:** Açık oturumdan çıkmadan yeni hesap eklenir (`-y auth`). Geçişte kabuk kısa süre kararır ve yeni ad görünür; kütüphane anlık görüntüsü ile profil önbelleği hemen çizilir, Epic listesi arkada sürer. Anlık görüntü yoksa paylaşılan `metadata` taranmaz (`[]` yazılır). Profil önbelleği varsa GraphQL zorlanmaz. Ayarlar Hesap bölümünde açılır; hesap satırında kimlik yazılmaz. Başarım satırında çözülemeyen Epic sandbox kimliği gösterilmez.
- **Kütüphane:** gizleme araması satırı gerçekten gizler (`display:flex` artık `[hidden]` kuralını ezmez). Hover kapak kutusunu büyütür, alttaki ad yerinde ve aynı renkte kalır. Mouse çekilince küçülme bitene kadar `contain` geri gelmez, animasyon kırpılmaz.
- **Profil başarımları:** Gizli sekmesi diğer sekmelerle aynı anda seçili kalmaz. Tümü, Platin, Devam eden veya Başlanmamış tıklanınca gizli liste aynı tıkta kapanır.
- **OpenCode / Gemini notu:** Flash modeller için kısa kurallar `docs/OPENCODE.md`, dosya haritası `docs/OPENCODE_MAP.md` içindedir. OpenCode ikisini kök `opencode.jsonc` ile yükler; `librarian` ve `deep-worker` `.opencode/agents/` altındadır. Gemini CLI `GEMINI.md` okur. Tasarım `AGENTS.md` §1–7 ve `docs/DESIGN_SYSTEM.md` dosyasındadır. Bu bölümün geri kalanı geçmiş kayıttır.
- **Sürüm 0.1.12:** Kütüphanede isteğe bağlı sayfalama (24/48/96) ve kapak altında oyun adı; ekran görüntüsü klasörü seçilebilir, klasör değişince mevcut görüntüler onayla taşınır, "Klasörü aç" düğmesi var; görsel sıkıştırma varsayılan açık. `v0.1.12` olarak yayınlanır.
...
- **Launcher güncellemesi:** yeni sürüm bulununca ve kurulum hazır olunca hem uygulama içi uyarı hem Windows bildirimi gider.
