...
const socialRoot = document.getElementById("social-root") as HTMLElement;
...
let socialError = "";
let socialActiveChatFriendId: string | null = null;
let socialPartyPrivacy: "invite" | "friends" | "public" = "invite";
let socialMicMuted = true;
let socialAudioDeafened = false;
let socialSearchQuery = "";
let socialShowAddModal = false;
let socialAddQuery = "";
let socialAddLoading = false;
let socialAddResult: EpicFriend | null | "not_found" = null;
...
let socialChatHistory: Record<string, SocialChatMessage[]> = {};
try {
  const saved = localStorage.getItem("efxlve-social-chats");
  if (saved) socialChatHistory = JSON.parse(saved);
} catch {}

function saveSocialChats(): void {
  try {
    localStorage.setItem("efxlve-social-chats", JSON.stringify(socialChatHistory));
  } catch {}
}
...
function updateSocialBadge(): void {
  ...
  const now = Date.now();
  const onlineCount = socialData.friends.filter((f) => {
    if (f.status === "ONLINE" || f.status === "AWAY") return true;
    if (f.last_online) {
      const diffMins = (now - new Date(f.last_online).getTime()) / 60000;
      return diffMins <= 30;
    }
    return false;
  }).length;
...
async function fetchSocialData(background = false): Promise<void> {
  if (!isTauri) return;
  if (!background) {
    socialLoading = true;
...
function formatFriendActivity(f: EpicFriend): { statusClass: string; statusLabel: string; isOnline: boolean } {
  if (!f.last_online) {
    return { statusClass: "offline", statusLabel: "Çevrimdışı", isOnline: false };
  }
  const diffMs = Date.now() - new Date(f.last_online).getTime();
  const diffMins = Math.floor(diffMs / 60000);
  const diffHours = Math.floor(diffMins / 60);
  const diffDays = Math.floor(diffHours / 24);

  if (diffMins <= 20) {
    return { statusClass: "online", statusLabel: "Çevrimiçi", isOnline: true };
  } else if (diffMins <= 60) {
    return { statusClass: "away", statusLabel: `${diffMins} dk önce`, isOnline: true };
  } else if (diffHours < 24) {
    return { statusClass: "offline", statusLabel: `${diffHours} sa önce`, isOnline: false };
  } else if (diffDays === 1) {
    return { statusClass: "offline", statusLabel: "Dün", isOnline: false };
  } else {
    return { statusClass: "offline", statusLabel: `${diffDays} gün önce`, isOnline: false };
  }
}
...
function renderSteamSidebarHeader(): string {
  ...
  const privacyText =
    ...
    : socialPartyPrivacy === "friends" ? "Sadece arkadaşlar"
    : "Herkese açık";
...
function renderSteamFriendsList(): string {
  ...
  const friends = socialData?.friends || [];
  const incoming = socialData?.incoming || [];
  ...
  if (socialSearchQuery.trim()) {
    ...
    filtered = filtered.filter(
      (f) => f.display_name.toLowerCase().includes(q) || (f.alias && f.alias.toLowerCase().includes(q)),
    );
  }
...
function sendActiveSocialChatMessage(textToSend?: string): void {
  if (!socialActiveChatFriendId) return;
  const input = document.getElementById("social-chat-input") as HTMLInputElement | null;
...
async function performSocialUserSearch(name: string): Promise<void> {
  if (!name.trim()) return;
...
async function performSendFriendInvite(targetAccountId: string): Promise<void> {
  try {
    await epicSendFriendRequest(targetAccountId);
    toast("Arkadaşlık isteği gönderildi! ✨", "ok");
    socialAddResult = null;
    socialShowAddModal = false;
    void fetchSocialData(true);
  } catch (err) {
    toast(`İstek gönderilemedi: ${String(err)}`, "err");
  }
}
...
document.addEventListener("click", (e) => {
  ...
  if (act === "toggle-social") {
    ...
    return;
  } else if (act === "social-quick-msg") {
    const qmsg = t.dataset.text;
    if (qmsg) sendActiveSocialChatMessage(qmsg);
    return;
  } else if (act === "social-search-add") {
    const addInp = document.getElementById("social-add-input") as HTMLInputElement | null;
    const name = (addInp?.value || socialAddQuery).trim();
    if (name) void performSocialUserSearch(name);
    return;
  } else if (act === "social-send-invite") {
    const targetId = t.dataset.id;
    if (targetId) void performSendFriendInvite(targetId);
    return;
  } else if (act === "social-accept-friend") {
    const targetId = t.dataset.id;
    if (targetId) void performSendFriendInvite(targetId);
    return;
  } else if (act === "social-decline-friend") {
    const targetId = t.dataset.id;
    if (targetId) void performRemoveFriend(targetId);
    return;
  } else if (act === "social-invite-party") {
    const name = t.dataset.name || "Arkadaş";
    toast(`${name} grubunuza davet edildi 🎮`, "ok");
    return;
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Enter") {
    ...
    if (activeEl && activeEl.id === "social-chat-input") {
      e.preventDefault();
      sendActiveSocialChatMessage();
      return;
    }
    if (activeEl && activeEl.id === "social-add-input") {
      e.preventDefault();
      const q = (activeEl as HTMLInputElement).value.trim();
      if (q) void performSocialUserSearch(q);
      return;
    }
...
document.addEventListener("input", (e) => {
  ...
  if (t && t.id === "social-add-input") {
    socialAddQuery = (t as HTMLInputElement).value;
    return;
  }
