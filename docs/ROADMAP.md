...
6. **İndirme davranışları (Faz C):** ✅ "Oyun oynarken indirmeleri duraklat" uygulandı (opsiyonel, varsayılan kapalı; `game-status` ile aktif indirme otomatik duraklatılıp oyun kapanınca devam ettirilir). Kalan: **zamanlanmış otomatik güncelleme** (arka plan zamanlayıcı + kuyruk kontrolü gerekir).
